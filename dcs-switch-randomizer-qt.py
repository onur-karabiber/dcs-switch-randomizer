...
LUA_SRC_DIR = os.path.join(THIS_DIR, "DSR")
...
def installed_version(scripts_dir):
    return read_version(os.path.join(scripts_dir, "DSR", "version.txt"))

APP_VERSION = exe_version()

# ── Export.lua helpers ────────────────────────────────────────────────────────
CR_BLOCK_START = "-- [DSR:begin]"
CR_BLOCK_END   = "-- [DSR:end]"
...
def copy_lua_files(dst_scripts):
    dst = os.path.join(dst_scripts, "DSR")
...
def is_installed(scripts_dir):
    return os.path.isfile(os.path.join(scripts_dir, "DSR", "core.lua"))

def config_path(scripts_dir):
    return os.path.join(scripts_dir, "DSR", "gui_config.json")
...
def json_defaults_dir(scripts_dir):
    ...
    return os.path.join(scripts_dir, "DSR", "json_defaults")
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Arka plan görseli
        bg_path = os.path.join(THIS_DIR, "DSR", "left-side-bg.png")
        ...
        # Fixed header (top margin only — content per screen)
        hdr = QWidget(); hdr.setObjectName("main")
        hdr_lay = QVBoxLayout(hdr)
        hdr_lay.setContentsMargins(20, 20, 20, 0)
    ...
    def _show_install_screen(self):
        ...
        # Logo
        logo_path = os.path.join(THIS_DIR, "DSR", "dsrlogo1.png")
        lbl_logo = QLabel()
        ...
        self.content.addWidget(lbl_logo)
        self.content.addSpacing(24)

        # Versiyon
        lbl_ver = QLabel(f"v{APP_VERSION}")
        lbl_ver.setObjectName("ver")
        lbl_ver.setAlignment(Qt.AlignCenter)
        self.content.addWidget(lbl_ver)
        self.content.addSpacing(24)

        head = QLabel("Installation"); head.setObjectName("head"); head.setAlignment(Qt.AlignCenter)
        info = QLabel(f"DCS Switch Randomizer will be installed to:\n\n"
                      f"{scripts_display}\\DSR\\")
    ...
    def _show_main_screen(self):
        ...
        # Banner
        banner_path = os.path.join(THIS_DIR, "DSR", "dsrbanner4.png")
        lbl_banner = QLabel()
        ...
        if os.path.isfile(banner_path):
            ...
            lbl_banner.setText("DCS SWITCH RANDOMIZER")
            lbl_banner.setObjectName("title")
        self.content.addSpacing(12)
        self.content.addWidget(lbl_banner)
        lbl_ver2 = QLabel(f"v{APP_VERSION}")
        lbl_ver2.setObjectName("ver")
        lbl_ver2.setAlignment(Qt.AlignCenter)
        self.content.addWidget(lbl_ver2)
        self.content.addSpacing(12)
        ...
        _ICON_BTNS = [
            ...
            ("\uE107", "iconbtn_uninst",   self._confirm_uninstall,  "Uninstall: remove DSR completely."),
        ]
    ...
    def _confirm_uninstall(self):
        ...
        if self._ask("Uninstall",
                     f"This will remove DCS Switch Randomizer completely.\n\n{extra}\n\nContinue?"):
            self._do_uninstall()

    def _do_uninstall(self):
        try:
            scripts = self.scripts_dir
            cr_dir  = os.path.join(scripts, "DSR")
            export  = export_lua_path(scripts)
            ...
            self._info("Uninstall complete", "DCS Switch Randomizer has been removed.")
            self.close()
        except Exception as e:
    ...
    def _do_reset_settings(self):
        ...
        try:
            count = 0
            cr_dir = os.path.join(self.scripts_dir, "DSR")
    ...
    def _json_search_dirs(self):
        dirs = [THIS_DIR]
        if self.scripts_dir:
            dirs.append(os.path.join(self.scripts_dir, "DSR"))
        return dirs
    ...
    def _do_export_settings(self):
        ...
        try:
            ...
            backup_name = f"DSR_Backup_{date.today().isoformat()}"
